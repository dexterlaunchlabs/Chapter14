...
typedef struct { NSUInteger row;
    NSUInteger column; } GridIndex;
...
- (void)initProperties;
- (void)drawBlockAtRow:(NSUInteger)row column:(NSUInteger)column; - (GridIndex)touchedGridIndexFromTouches:(NSSet *)touches;
...
- (void)initProperties {
    ...
    gapSize = CGSizeMake(5, 5); selectedBlockIndex.row = NSNotFound; selectedBlockIndex.column = NSNotFound; highlightColor = [UIColor blackColor];
}
